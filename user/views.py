...
def index(request):
    ...
    opdata=myproduct.objects.all().filter(total_discount__gte=50)
...
def signin(request):
    if request.method=="POST":
        ...
        if x==1:
            ...
            return HttpResponse("<script>alert('login successful..');location.href='/user/index/'</script>")
        else:
            return HttpResponse("<script>alert('Your user id or password are incorrect');location.href='/user/signin/'</script>")
...
def mycart(request):
    ...
    if user:
        ...
        if qt>0:
            ...
            return HttpResponse("<script>alert('please incerase your cart items');location.href='/user/product/'</script>")
...
def indexcart(request):
    ...
    if user:
        ...
        if qt>0:
            ...
            return HttpResponse("<script>alert('Please Incerase Your Cart Items..');location.href='/user/index/'</script>")
...
def orderslist(request):
    ...
    ddata=myorders.objects.filter(userid=user,status="Deliverd").order_by('-id')
    if oid is not None:
